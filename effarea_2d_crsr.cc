...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    if (isnan(weight1) != 0 || isinf(weight1) != 0)
      weight1 = 0.;
    harea_p0->Fill(log10(energy), theta, weight1);
    if (log10(energy) > 3.36 && log10(energy) < 3.4)
      harea_p0_59->Fill(theta, weight1);
    if (!(nTop / (vcxPE[6] + 0.01) > copt && dAngle < 7.557))
      continue;
    test3 = test3 + weight;
    ...
    if (isnan(weight2) != 0 || isinf(weight2) != 0)
      weight2 = 0.;
  ...
  //  for(int i=238;i<239;i++)
  {
    ...
    for (d_zen = round(d_zen_min); d_zen <= round(d_zen_max); d_zen++)
    {
      ...
      if (isnan(S_arc) != 0 || isinf(S_arc) != 0)
        S_arc = 0.;
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    if (log10(energy) > 3.36 && log10(energy) < 3.4)
      harea_p1_59->Fill(theta, weight1);
    tmp_w = weight / expo_zen[int(theta / zen_bin_width)];
    if (isnan(tmp_w) != 0 || isinf(tmp_w) != 0)
      tmp_w = 0.;
    weight1_itgr += tmp_w;
    if (!(nTop / (vcxPE[6] + 0.01) > copt))
      continue;
    test3 = test3 + weight;
    tmp_w = weight / expo_zen[int(theta / zen_bin_width)];
    if (isnan(tmp_w) != 0 || isinf(tmp_w) != 0)
