...
//*/ #define Tibet_Lati      30.102                   /* Latitude of Tibet
//(deg.) */ #define mjd_st 56789.0    /* a arbitrary virtual start time to
// calculate the position of point sources   */ static  float   n[NLST][NZEAZ];
...
void wcda_ang_maxsigma(double day_num, double *theta_maxsigma, double *maxsigma,
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    printf("%s  output(.dat)  emid_ga  emax_ga  days  [1 for .root]  [1 for "
           "monitor mode]  (use log10(emid_ga/emax_ga))  set emax_ga=0 for "
           "default: 14.\n",
           argv[0]);
    exit(0);
  }
  ...
  for (int tmp_i = 0; tmp_i < NZE; tmp_i++) {
    ...
    //    area_nze[tmp_i] = PI * ( thetamax * thetamax - thetamin * thetamin )
    //    / NAZ0[tmp_i];
    area_nze[tmp_i] = 2 * PI * (cos(thetamin * D2R) - cos(thetamax * D2R)) *
                      R2D * R2D / NAZ0[tmp_i];
  ...
  for (i = 0; i < NZE; i++) {
    ...
    for (n_bin = 0; n_bin < N_int; n_bin++) {
      ...
      // 0.023293=10^0.01-1
      //      sum_bin[i][n_bin] = bin_area * wcda_filter_p(emid + 0.005 +
      //      0.01*n_bin);
      sum_int += bin_area *
      ...
      // if(i==1.0&&n_bin==0) {printf("!!! CR_intens_p1 bin_h bin_area sum_int
      // %e %f %f %f\n", CR_intens_p1(emid + 0.005 + 0.01*n_bin),  bin_h,
      // bin_area, sum_int);}
    }
    ////  grid_sr is the same to area_nze[];
    //  grid_sr = 2.0 * PI * ( cos(i*hori_sys_width*D2R) -
    //  cos((i+1)*hori_sys_width*D2R) ) / NAZ0[i];

    evnum_p[i] =
        sum_int * area_nze[i] * D2R * D2R * dt_time * days * 86400. * 0.9354;
    // 0.9354 is a parameter has no physical meaning but to calibrate to
    // reproduce a correct bg events number according to Yao's data!!!!!!!!!
    ...
    for (i = 0; i < NZE; i++) {
      ...
      for (j = 0; j < NAZ0[i]; j++) {
        if (evnum_p[i] > 50) {
          ...
          //            tmp_stream1 =
          //            rand_gauss((double)evnum_p[i],(double)sqrt(evnum_p[i]));
  ...
  if (PS_INPUT == 1) {
    ...
    const double rad_over_ang_reso = 3.0;
    double ga_max[300] = {0};
    ...
    const int PS_num = sizeof(PS_TEVCAT_ZY) / (sizeof(double) * 7);
    ...
    wcda_ang_maxsigma(days, &ang_opt, &maxsigma, &keepratio_ga, &keepratio_cr,
                      &num_ga, &num_cr);
    printf("(theta_maxsigma, maxsigma, keepratio_ga, keepratio_cr, num_ga, "
           "num_cr) = (%g, %g, %g, %g, %g, %g)\n",
           ang_opt, maxsigma, keepratio_ga, keepratio_cr, num_ga, num_cr);
    fprintf(fp_log,
            "(theta_maxsigma, maxsigma, keepratio_ga, keepratio_cr, num_ga, "
            "num_cr) = (%g, %g, %g, %g, %g, %g)\n",
            ang_opt, maxsigma, keepratio_ga, keepratio_cr, num_ga, num_cr);
    //!!!!!!!!!!!!!!!!!!!!!modified!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ang_opt = 0.557;
    ...
    for (i_PS = 0; i_PS < PS_num; i_PS++) {
      for (i = 0; i < NZE; i++) {
        ...
        phi = 0.;
        for (n_bin = 0; n_bin < N_int; n_bin++) {
          bin_h = SOURCE_intens_ZY(i_PS, emin_ga + 0.005 + 0.01 * n_bin);
          ...
          //        sum_bin[i][n_bin] = bin_area * wcda_eff_area_gamma(emid +
          //        0.005 + 0.01*n_bin, (i+0.5)*equa_sys_width) *
          //        wcda_filter_ga(emid + 0.005 + 0.01*n_bin);
          ga_num[i_PS][n_bin] +=
              bin_area *
    ...
    for (i_PS = 0; i_PS < PS_num; i_PS++) { // TODO
      // for (i_PS = 30; i_PS < 31; i_PS++) {
      for (isr = 0; isr < NLST; isr++) {
        lst = (isr + 0.5) * hori_sys_width;
        equator_horizon_lst(lst, PS_TEVCAT_ZY[i_PS][0], PS_TEVCAT_ZY[i_PS][1],
                            &ZEN, &AZI);
        ...
        imax = (i0 + i_err) < (NZE - 1) ? (i0 + i_err) : (NZE - 1);
        for (i = imin; i <= imax; i++) {
          j0 = floor(AZI * NAZ0[i] / 360.);
          jmin = (j0 - i_err) > 0 ? (j0 - i_err) : 0;
          jmax = (j0 + i_err) < (NAZ0[i] - 1) ? (j0 + i_err) : (NAZ0[i] - 1);
          ...
          fseek(fp_out, (NZEAZ * isr + NAZ[i] + jmin) * sizeof(float),
                SEEK_SET);
          ...
          for (j = jmin; j <= jmax; j++) {
            ...
            if (tmp_num_ga > 50) {
              tmp_num_ga =
                  rand_gauss((double)tmp_num_ga, (double)sqrt(tmp_num_ga));
            } else {
            ...
            // tmp_num_ga = evnum_ga[i_PS][i0]; // FIXME
            if (theta < ang_opt) {
              bg_max_tmp += buffer1[j - jmin];
              ga_max[i_PS] += tmp_num_ga;
              S_ang_opt_raw += area_nze[i];
            }
            buffer2[j - jmin] = tmp_num_ga + buffer1[j - jmin]; // TODO
            // buffer2[j - jmin] = tmp_num_ga; // TODO
            if (buffer2[j - jmin] < 1e-5)
              zero_num[i_PS]++;
          }
          fseek(fp_out, (NZEAZ * isr + NAZ[i] + jmin) * sizeof(float),
                SEEK_SET);
          // fseek(fp_out, -(jmax - jmin + 1) * sizeof(float), SEEK_CUR);//FIXME
      ...
      // exit(0);
      printf("\n( i_PS, reso0_source_event, source_event_within_ang_opt, "
             "bg_event_within_ang_opt, raw_sigma, grids have zero event "
             "numbers ) = ( %4d, %f, %f, %f, %f, %f )\n",
             i_PS, ga_max_reso0[i_PS], ga_max[i_PS], bg_max[i_PS],
             ga_max[i_PS] / sqrt(bg_max[i_PS]), zero_num[i_PS]);
      fprintf(fp_log,
              "( i_PS, reso0_source_event, source_event_within_ang_opt, "
              "bg_event_within_ang_opt, raw_sigma, grids have zero event "
              "numbers ) = ( %4d, %f, %f, %f, %f, %f )\n",
    ...
    if (isroot == 1) {
      ...
      TH2D *hradec_ps =
          new TH2D("hradec_in_ps", "RADEC dis.", 180, 0., 360., 135, -45., 90.);
      ...
      for (rewind(fp_out), rewind(fp_nb), isr = 0; isr < NLST; isr++) {
        for (rewind(fp_grid_ra_dec), i = 0; i < NZE; i++) {
          ...
          for (j = 0; j < NAZ0[i]; j++) {
            ...
            hradec_ps->Fill(ra, dec, buffer1[j]);
...
double bigaussian_mean(double theta, double sigma) {
  double thetamin = theta - hori_sys_width / sqrt(PI) > 0
                        ? theta - hori_sys_width / sqrt(PI)
                        : 0;
...
double wcda_ang(double ep) {
  ...
  if (ep <= exb[0])
    ...
    while (ep > exb[i])
      i++;
    theta = (angoptb[i] - angoptb[i - 1]) / (exb[i] - exb[i - 1]) *
                (ep - exb[i - 1]) +
            angoptb[i - 1];
  }
...
double wcda_theta_dist(double theta) {
  ...
  ring_area = 2. * PI *
              (cos((bin_num - 1) * 0.01 * D2R) - cos(bin_num * 0.01 * D2R)) *
              R2D * R2D;
...
double wcda_theta_dist1(double theta) {
  ...
  bin_num_le = (0 > (bin_num - hori_sys_width / (2 * 0.01)))
                   ? 0
                   : (bin_num - hori_sys_width / (2 * 0.01));
  bin_num_ue = (700 < (bin_num + hori_sys_width / (2 * 0.01)))
                   ? 700
                   : (bin_num + hori_sys_width / (2 * 0.01));
  //  orig_bin_cont =
  //  ((TH1F*)file_ang->Get("hpol_rho"))->GetBinContent(bin_num);
  orig_bin_cont =
      ((TH1F *)file_ang->Get("hpol_rho"))->Integral(bin_num_le, bin_num_ue) /
      (bin_num_ue - bin_num_le + 1);
...
void wcda_ang_maxsigma(double day_num, double *theta_maxsigma, double *maxsigma,
  ...
  *num_cr *= day_num;
  *keepratio_ga =
      (((TH1F *)file_ang->Get("hpol_smth"))->Integral(1, n_maxsigma)) /
      (((TH1F *)file_ang->Get("hpol_smth"))->Integral(1, 700));
  *keepratio_cr =
      (((TH1F *)file_ang->Get("hpol_cr_smth"))->Integral(1, n_maxsigma)) /
      (((TH1F *)file_ang->Get("hpol_cr_smth"))->Integral(1, 700));
}
