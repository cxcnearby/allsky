...
double CR_intens_p1(double ep) {
  ...
  ep = pow(10.0, ep - 9.0);
  crspec = 1e-13 * (k1 * pow(ep, -a1) * exp(-ep / (z * ec1)) +
                    k2 * pow(ep, -a2) * exp(-ep / (z * ec2)) +
                    k3 * pow(ep, -a3) * exp(-ep / (z * ec3)));
...
double CR_intens_p3(double ep) {
  ...
  ep = pow(10.0, ep - 9.0);
  crspec = 1e-13 *
           (k1 * pow(ep, -a1) * exp(-ep / (z * ec1)) +
            k2 * pow(ep, -a2) * exp(-ep / (z * ec2)) +
            k3 * pow(ep, -a3) * exp(-ep / (z * ec3))); // ev^-1 cm^-2 s^-1 sr^-1
  return crspec;
}
...
double CR_intens_all(double ep) {
  return CR_intens_p1(ep) + CR_intens_p2(ep) + CR_intens_p3(ep) +
         CR_intens_p4(ep) + CR_intens_p5(ep);
}
...
double CR_itgr_intens_all(double ep1, double ep2) {
  ...
  double sum = 0.;
  for (i = 0; i < n; i++) {
    area = CR_intens_all(ep1 + 0.005 + 0.01 * n) * 0.023293 *
           pow(10.0, ep1 + 0.01 * n);
    sum += area;
  }
  return sum;
...
double SOURCE_intens(int i, double eg) {
  ...
  if (PS_TEVCAT[i][5] < 1e-3) {
    return 1e-12 * PHI0 * pow((eg / E0), INDEX);
  } else {
    return 1e-12 * PHI0 * pow((eg / E0), INDEX) * exp(-eg / ECUT);
  }
}
...
{
  eg = pow(10.0,eg - 12.0);
  double INDEX = PS_TEVCAT_ZY[i][3];
  double flux = (0.1*exp(2.30259*INDEX) - 0.001*exp(6.90776*INDEX))/(1. - INDEX);
  double PHI0 = PS_TEVCAT_ZY[i][2]/flux;
  double ECUT = pow(10,PS_TEVCAT_ZY[i][6] - 3.);
  double result;
  if(eg<=ECUT) {result = 1e-12 * PHI0 * pow(eg,0.-INDEX);}
  else {result = 1e-12 * PHI0 * pow(ECUT,PS_TEVCAT_ZY[i][5]-PS_TEVCAT_ZY[i][3]) * pow(eg,0. - PS_TEVCAT_ZY[i][5]);}
