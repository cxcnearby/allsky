...
void equator_horizon_lst(double lst, double ras, double dec, double *zen,
                         double *azi) {
  ...
  if (H < 0)
    H += 360.0;
  sa = sin(dec * deg_rad) * sin(LHAASO_Lati * deg_rad) +
       cos(dec * deg_rad) * cos(LHAASO_Lati * deg_rad) * cos(H * deg_rad);
  a = asin(sa) * rad_deg;
  cA = (sin(dec * deg_rad) - sin(LHAASO_Lati * deg_rad) * sa) /
       (cos(LHAASO_Lati * deg_rad) * cos(a * deg_rad));
...
void equator_horizon_lst_ybj(double lst, double ras, double dec, double *zen,
                             double *azi) {
  ...
  if (H < 0)
    H += 360.0;
  sa = sin(dec * deg_rad) * sin(YBJ_Lati * deg_rad) +
       cos(dec * deg_rad) * cos(YBJ_Lati * deg_rad) * cos(H * deg_rad);
  a = asin(sa) * rad_deg;
  cA = (sin(dec * deg_rad) - sin(YBJ_Lati * deg_rad) * sa) /
       (cos(YBJ_Lati * deg_rad) * cos(a * deg_rad));
...
double distance_equatorial(double ra1, double dec1, double ra2, double dec2) {
  if (fabs(ra1 - ra2) < 1e-8 && fabs(dec1 - dec2) < 1e-8)
    return 0;
  else
...
double distance_horizontal(double zen1, double azi1, double zen2, double azi2) {
  if (fabs(zen1 - zen2) < 1e-8 && fabs(azi1 - azi2) < 1e-8)
    return 0;
  else
