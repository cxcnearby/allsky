...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    printf("%s  input(.dat)  [1 for monitor mode]\n", argv[0]);
    exit(0);
  }
  if (argc >= 3)
    ismon = atoi(argv[2]);
  strcpy(filename, argv[1]);
  if ((fp_in = fopen(strcat(argv[1], ".dat"), "rb")) == NULL) {
    printf("cannot open input file\n");
    exit(0);
  }
  strcpy(argv[1], filename);
  if ((fp_out = fopen(strcat(argv[1], "c.dat"), "wb")) == NULL) {
  ...
  for (isr = 0; isr < NLST; isr++) {
    if (ismon == 1)
      printf("reading data!   %3d%% \r", int(isr * 100 / NLST));
    for (k = 0; k < NZEAZ; k++) {
  ...
  for (i = 0; i < NZE; i++) {
    if (ismon == 1)
      printf("calibrating!   %3d%% \r", int(i * 100 / NZE));
    n_b = 0.;
    k0 = k;
    for (j = 0; j < NAZ0[i]; j++) {
      n_b += eff[k];
  ...
  for (isr = 0; isr < NLST; isr++) {
    if (ismon == 1)
      printf("correcting data!   %3d%% \r", int(isr * 100 / NLST));
    for (k = 0; k < NZEAZ; k++) {
      fread(&n0, sizeof(n0), 1, fp_in);
      n = n0 / eff[k];
