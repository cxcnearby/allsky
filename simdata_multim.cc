...
int main(int argc, char *argv[]) {
  ...
  if (PS_INPUT == 1) {
    ...
    for (i_PS = 30; i_PS < 31; i_PS++) {
      for (isr = 2900; isr < NLST; isr=isr+1) {
        ...
        bg_max_tmp = 0.;
        S_ang_opt_raw = 0.;
        j0 = floor(AZI * NAZ0[i0] / 360.);
        //printf("%d,%d\n",i0,j0);
        float buffer1;
        // float *buffer1 = new float[1]();
        float buffer2;
        // float *buffer2 = new float[1]();
        fseek(fp_out, (NZEAZ *isr + NAZ[i0] + j0) * sizeof(float), SEEK_SET);
        fread(&buffer1, sizeof(float), 1, fp_out);
        fprintf(fp_log,"%f\n",buffer1);
        tmp_num_ga = evnum_ga[i_PS][i0]; // FIXME
        buffer2 = tmp_num_ga + buffer1;  // TODO

        fseek(fp_out, (NZEAZ *isr + NAZ[i0] + j0) * sizeof(float), SEEK_SET);
        // fseek(fp_out, -(jmax - jmin + 1) * sizeof(float), SEEK_CUR);//FIXME
        fwrite(&buffer2, sizeof(float), 1, fp_out);
        fprintf(fp_log, "%d, %f, %f, %d, %d, %f\n", isr, ZEN, AZI, i0, j0, buffer2);
        fseek(fp_out, (NZEAZ *isr + NAZ[i0] + j0) * sizeof(float), SEEK_SET);
        fread(&buffer2, sizeof(float), 1, fp_out);
        fprintf(fp_log,"%ld %f\n",ftell(fp_out),buffer2);
        // delete[] buffer1;
        // delete[] buffer2;
      ...
      for (rewind(fp_out), rewind(fp_nb), isr = 0; isr < NLST; isr++) {
        for (rewind(fp_grid_ra_dec), i = 0; i < NZE; i++) {
          ...
          for (j = 0; j < NAZ0[i]; j++) {
            ...
            fread(&buffer1, sizeof(float), 1, fp_out);
            //            fread(&tmp_stream1,sizeof(tmp_stream1),1,fp_out);
            hzen_ps->Fill((i + 0.5) * hori_sys_width, buffer1);
            // if(tmp_stream1>1e-6) printf("%f
            // %f\n",(i+0.5)*hori_sys_width,tmp_stream1);
            hazim_ps->Fill((j + 0.5) * 360. / NAZ0[i], buffer1);
            ra = (ra + isr * hori_sys_width) < 360.0
                     ? (ra + isr * hori_sys_width)
                     : (ra + isr * hori_sys_width - 360.0);
            hra_ps->Fill(ra, buffer1);
            hdec_ps->Fill(dec, buffer1);
            hradec_ps->Fill(ra, dec, buffer1);
            hsig_ps->Fill((buffer1) / sqrt(tmp_stream2));
          }
          //          fread(&tmp_stream2,sizeof(tmp_stream2),1,fp_nb);
