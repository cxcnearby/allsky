...
void sig_correct(int i0, int j0, double r_smooth, double grid_width,
                 double *sig, double *intens, double *Nsig, double *Nbg,
                 int *Nsmthbin);
void wcda_ang_maxsigma(double *theta_maxsigma, double *maxsigma,
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    printf("%s  input(d.dat)  ediff  [reso1]  [reso2]  [t_reuse]  [1 for "
           "monitor mode]  (use log10(ediff))  set Reso1=0 for default\n",
           argv[0]);
    exit(0);
  }
  ediff = atof(argv[2]);
  //  reso1=1.5852*wcda_ang(ediff);
  wcda_ang_maxsigma(&reso1, &maxsigma, &keepratio_ga, &keepratio_cr, &num_ga,
                    &num_cr);
  ...
  for (t = 0; t < t_reuse; t++) {
    ...
    for (i = 0; i < NRA; i++) {
      if (ismon == 1)
        printf("%d/%d    initiating!   %3d%% \r", t, t_reuse,
               int(i * 100 / NRA));
      for (j = 0; j < NDEC; j++) {
  ...
  TH2D *h8021 =
      new TH2D("h8021", "Intens", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  TH2D *h8020 =
      new TH2D("h8020", "Signi", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  ...
  for (j = 0; j < NDEC;
       j++) { // for reusing the selected grid when DEC is same.
    if (ismon == 1)
      printf("point source smooth set!   %3d%%\r", int(j * 100 / NDEC));
    for (i = 0; i < NRA; i++) {
      hdec->Fill((j + 0.5) * equa_sys_width + DEC_MIN, a[i][j]);
      sig_correct(i, j, reso1, equa_sys_width, &signi[i][j], &intens[i][j],
                  &num_sig, &num_bg, &num_bin);
      if (isnan(signi[i][j]) != 0 || isinf(signi[i][j]) != 0)
        signi[i][j] = 0;
      if (sig_max < signi[i][j] &&
          fabs((i + 0.5) * equa_sys_width - CRAB[0]) < 2 &&
      ...
      h8020->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN, signi[i][j]);
      h10->Fill(signi[i][j]);
      hsig->Fill(signi[i][j]);
  ...
  TH2D *h7021 =
      new TH2D("h7021", "Intens", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  TH2D *h7020 =
      new TH2D("h7020", "Signi", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  TH2D *h5s = new TH2D("h5s", "Signi", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  for (j = 0; j < NDEC; j++) {
    if (ismon == 1)
      printf("large area smooth set!   %3d%%\r", int(j * 100 / NDEC));
    for (i = 0; i < NRA; i++) {
      sig_correct(i, j, reso2, equa_sys_width, &signi[i][j], &intens[i][j],
                  &num_sig, &num_bg, &num_bin);
      h7020->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN, signi[i][j]);
      h7021->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN, intens[i][j]);
      if (fabs(intens[i][j] - 1) / signi[i][j] < 0.0002)
        h5s->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN, intens[i][j]);
    }
  }
  ...
  TH2D *h6021 =
      new TH2D("h6021", "Intens", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  TH2D *h6020 =
      new TH2D("h6020", "Signi", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  double intensi0, err0;
  for (i = 0; i < NRA; i++) {
    if (ismon == 1)
      printf("ps - area smooth  set!   %3d%%\r", int(i * 100 / NRA));
    for (j = 0; j < NDEC; j++) {
      intensi0 = h8021->GetBinContent(i + 1, j + 1) -
                 h7021->GetBinContent(i + 1, j + 1);
      err0 = (h8021->GetBinContent(i + 1, j + 1) - 1) /
             h8020->GetBinContent(i + 1, j + 1);
      //  printf("intenis %g\n",err0);
