...
int main(int argc, char *argv[]) {
  ...
  if ((fp_in = fopen(strcat(argv[1], "c.dat"), "rb")) == NULL) {
    printf("cannot open input file\n");
    exit(0);
  }
  strcpy(argv[1], filename);
  if ((fp_out = fopen(strcat(argv[1], "d.dat"), "wb+")) == NULL) {
  ...
  strcpy(argv[1], filename);
  if ((fp_na = fopen(strcat(argv[1], "_tmp_na"), "wb+")) == NULL) {
  ...
  TH2D *hnoff =
      new TH2D("hnoff", "N_off", NRA, 0., 360., NDEC, DEC_MIN, DEC_MAX);
  ...
  for (i = 0; i < NRA; i++) {
    if (ismon == 1)
      printf("RA-DEC map!   %3d%% \r", int(i * 100 / NRA));
    for (j = 0; j < NDEC; j++) {
      //		   printf("%f\n",a[j][i]);
      ...
      h8020->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN, sig);
      h8021->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN,
                  b[i][j] == 0 ? 1 : a[i][j] / b[i][j]);
      hnon->Fill((i + 0.5) * equa_sys_width,
                 (j + 0.5) * equa_sys_width + DEC_MIN, a[i][j] - b[i][j]);
      hnoff->Fill((i + 0.5) * equa_sys_width,
                  (j + 0.5) * equa_sys_width + DEC_MIN, b[i][j]);
      h10->Fill(sig);
      hsig->Fill(sig);
